...
function App() {
  ...
  // --- HÀM UPDATE WINNER (ĐÃ SỬA LỖI TÍNH ĐIỂM XIN THUA) ---
  const updateGameWinner = async (winnerColor, reason = "checkmate") => {
    if (gameId) {
      ...
      // Logic mới: Không cần check 'winnerColor === playerColor' nữa
      // Bất kỳ ai kích hoạt hàm này (người thắng hoặc người thua) đều sẽ cập nhật điểm cho cả 2
      const winnerId = winnerColor === 'r' ? gameData.redPlayerId : gameData.blackPlayerId;
      const loserId = winnerColor === 'r' ? gameData.blackPlayerId : gameData.redPlayerId;

      if (winnerId && loserId) {
          // Cộng thắng
          const winRef = doc(db, "users", winnerId);
          updateDoc(winRef, { wins: increment(1) });
          // Cộng thua
          const loseRef = doc(db, "users", loserId);
          updateDoc(loseRef, { losses: increment(1) });
      }
    }
  };
  ...
  const handlePieceClick = (targetPiece) => { if (winner) return; if (playerColor === 'spectator') return; if (gameId) { if (turn !== playerColor) return; if (!selectedPiece && targetPiece.color !== playerColor) return; if (selectedPiece && targetPiece.color !== playerColor && turn !== playerColor) return; } if (!selectedPiece) { if (targetPiece.color !== turn) return; setSelectedPiece(targetPiece); return; } if (targetPiece.id === selectedPiece.id) { setSelectedPiece(null); return; } if (targetPiece.color === selectedPiece.color) { if (targetPiece.color === turn) setSelectedPiece(targetPiece); return; } if (!isValidMove(selectedPiece, targetPiece.x, targetPiece.y, pieces)) return; if (willCauseSelfCheck(selectedPiece, targetPiece.x, targetPiece.y, pieces)) return; executeMove(selectedPiece, targetPiece.x, targetPiece.y, true); };
  const handleSquareClick = (x, y) => { if (winner) return; if (playerColor === 'spectator') return; if (!selectedPiece) return; if (gameId && turn !== playerColor) return; if (!isValidMove(selectedPiece, x, y, pieces)) return; if (willCauseSelfCheck(selectedPiece, x, y, pieces)) return; executeMove(selectedPiece, x, y, false); };
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 items-center lg:items-start animate-fade-in w-full max-w-6xl px-2">
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="mb-2 flex flex-wrap justify-between items-center w-full" style={{ maxWidth: boardWidth }}>
                ...
                <div className="flex flex-col items-center gap-1 flex-1"> <div className="px-3 py-1 rounded bg-slate-700 text-white border border-slate-600 shadow text-xs"> Phòng: <span className="text-yellow-400 font-mono font-bold select-all">{gameId}</span> </div> <div className={`px-3 py-1 rounded font-bold text-xs border ${playerColor === 'r' ? 'bg-red-900 text-red-100 border-red-500' : playerColor === 'b' ? 'bg-black text-gray-300 border-gray-600' : 'bg-blue-900 text-blue-200 border-blue-500'}`}> {playerColor === 'r' ? 'BẠN LÀ ĐỎ' : playerColor === 'b' ? 'BẠN LÀ ĐEN' : 'BẠN LÀ KHÁN GIẢ'} </div> </div>
                <div className="flex flex-col items-center w-20"> <div className="flex items-center gap-1 mb-1"> <Timer time={redTime} isActive={turn === 'r' && !winner} /> <div className="w-2 h-2 bg-red-600 rounded-full border border-red-300"></div> </div> {turn === 'r' && !winner && <div className="text-red-400 font-mono font-bold text-[10px] animate-pulse">({currentMoveTime}s)</div>} </div>
            </div>
            ...
            {drawReq && !winner && <div className="absolute top-20 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white p-4 rounded-xl border-2 border-yellow-500 shadow-2xl z-50 flex flex-col items-center gap-3 animate-bounce w-64"> <h3 className="font-bold text-yellow-400 text-center">🤝 CẦU HÒA?</h3> <div className="flex gap-2 w-full"> <button onClick={handleAcceptDraw} className="flex-1 bg-green-600 py-1 rounded text-sm">OK</button> <button onClick={handleRejectDraw} className="flex-1 bg-red-600 py-1 rounded text-sm">Hủy</button> </div> </div>}
